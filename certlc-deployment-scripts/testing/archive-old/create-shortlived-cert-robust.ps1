...
if (-not $config) {
    Write-Host "❌ Could not load configuration from .env file" -ForegroundColor Red
...
try {
    ...
    # Step 3: Test network connectivity to Key Vault
    ...
    try {
        ...
        if (-not $kvTest) {
            ...
            Write-Host "   💡 This may be a network connectivity issue on DC01" -ForegroundColor Yellow
        ...
        Write-Host "   🔧 Check internet connectivity and DNS resolution" -ForegroundColor Yellow
        throw
    }
    
    # Step 4: Import to Key Vault with retry logic
    Write-Host "`n4. Importing to Key Vault..." -ForegroundColor Yellow
    ...
    while (-not $importSuccess -and $retryCount -lt $maxRetries) {
        try {
            ...
            if ($_.Exception.Message -like "*inProgress*") {
                ...
                try {
                    $importResult = Get-AzKeyVaultCertificate -VaultName $KeyVaultName -Name $CertificateName
                }
                catch {
                    Write-Host "   ℹ️  Could not retrieve certificate details" -ForegroundColor Gray
                }
            }
            else {
    ...
    # Step 5: Add recipient tag for renewal automation
    Write-Host "`n5. Adding recipient tag for renewal automation..." -ForegroundColor Yellow
    ...
    while (-not $tagSuccess -and $tagRetryCount -lt $maxRetries) {
        try {
            ...
            Set-AzKeyVaultCertificateAttribute -VaultName $KeyVaultName -Name $CertificateName -Tag @{"recipient" = $currentUser} -ErrorAction Stop | Out-Null
            $tagSuccess = $true
    ...
    if ($_.Exception.Message -like "*could not be resolved*") {
        ...
        Write-Host "   - DC01 may have intermittent internet connectivity" -ForegroundColor White
        Write-Host "   - Check network settings and try again" -ForegroundColor White
        Write-Host "   - Consider running from a different machine with stable internet" -ForegroundColor White
    }
}
finally {
    # Cleanup
    ...
    Get-ChildItem -Path "Cert:\CurrentUser\My" | Where-Object { $_.Subject -eq "CN=$CertificateName" } | Remove-Item -ErrorAction SilentlyContinue
...
Write-Host "`n💡 NETWORK TROUBLESHOOTING TIPS:" -ForegroundColor Cyan
Write-Host "If you see DNS resolution errors:" -ForegroundColor Yellow
Write-Host "1. Check internet connectivity: ping 8.8.8.8" -ForegroundColor Gray
Write-Host "2. Flush DNS cache: ipconfig /flushdns" -ForegroundColor Gray
Write-Host "3. Test Key Vault endpoint: nslookup $KeyVaultName.vault.azure.net" -ForegroundColor Gray
Write-Host "4. Check proxy/firewall settings" -ForegroundColor Gray
Write-Host "5. Try running from a different machine with stable internet" -ForegroundColor Gray