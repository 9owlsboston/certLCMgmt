...
function certlcworkflow {
    ...
    if ($WebhookData) { # Check that Webhookdata is not null
        ...
        if ($continue) {
            ...
            # IDEMPOTENCY CHECK - Prevent multiple renewals in short time
            $lastModified = $cert.Attributes.Updated
            $twoHoursAgo = (Get-Date).AddHours(-2)
            
            if ($lastModified -gt $twoHoursAgo) {
                Write-Output "Certificate $ObjectName was recently updated at $lastModified. Skipping renewal to prevent runaway creation."
                Write-Output "Current time: $(Get-Date). Certificate was updated less than 2 hours ago."
                return
            }
            
            # EXPIRATION THRESHOLD CHECK - Only renew certificates expiring within N days
            try {
                $renewalThresholdDays = [int](Get-AutomationVariable -Name 'CertRenewalThresholdDays' -ErrorAction Stop)
                Write-Output "Using configured renewal threshold: $renewalThresholdDays days"
            } catch {
                $renewalThresholdDays = 30  # Default: Only renew certificates expiring within 30 days
                Write-Output "Using default renewal threshold: $renewalThresholdDays days (CertRenewalThresholdDays variable not set)"
            }
            
            $expirationDate = $cert.Attributes.Expires
            $currentDate = Get-Date
            $daysUntilExpiry = ($expirationDate - $currentDate).Days
            
            Write-Output "Certificate expires: $expirationDate"
            Write-Output "Current date: $currentDate" 
            Write-Output "Days until expiry: $daysUntilExpiry"
            Write-Output "Renewal threshold: $renewalThresholdDays days"
            
            if ($daysUntilExpiry -gt $renewalThresholdDays) {
                Write-Output "Certificate $ObjectName expires in $daysUntilExpiry days, which is more than the renewal threshold of $renewalThresholdDays days."
                Write-Output "Skipping renewal. Certificate will be renewed when it's within $renewalThresholdDays days of expiration."
                return
            }
            
            Write-Output "Certificate $ObjectName expires in $daysUntilExpiry days, which is within the renewal threshold. Proceeding with renewal."
            
            # JOB LOCKING - Prevent concurrent executions
            ...
            try {
                ...
                if ((Get-Date) - $lockTime -lt [TimeSpan]::FromMinutes(30)) {
                    Write-Output "Certificate renewal already in progress (locked at $lockTime). Exiting to prevent concurrent execution."
                    return
                }
            } catch {
                # No lock exists, proceed
                Write-Output "No existing renewal lock found. Proceeding with renewal."
            }

            # Set renewal lock
            try {
                Set-AutomationVariable -Name $lockVariable -Value (Get-Date).ToString() -Encrypted $false
                Write-Output "Set renewal lock for certificate $ObjectName at $(Get-Date)"
            } catch {
                Write-Output "Warning: Could not set renewal lock variable. Proceeding anyway."
            }
            ...
            if ($CSR -eq $null) {
                ...
                try {
                    # Get configurable certificate validity period
                    try {
                        ...
                        # Default based on current industry compliance requirements
                        $currentYear = (Get-Date).Year
                        if ($currentYear -ge 2029) {
                            $validityMonths = 1.5  # 47 days = ~1.5 months (CA/B Forum 2029 requirement)
                        } elseif ($currentYear -ge 2027) {
                            $validityMonths = 3    # 100 days = ~3 months (CA/B Forum 2027 requirement)
                        } elseif ($currentYear -ge 2026) {
                            $validityMonths = 6    # 200 days = ~6 months (CA/B Forum 2026 requirement)
                        } else {
                            $validityMonths = 12   # Current standard (398 days)
                        }
                        Write-Output "Using automatic validity based on year $currentYear: $validityMonths months (CertificateValidityMonths variable not set)"
                    }
                    ...
                    Write-Output "Certificate policy created with $validityMonths months validity (approximately $(($validityMonths * 30)) days)"
...
"@
    ...
    # CLEANUP - Release renewal lock after successful completion
    if ($ObjectName) {
        $lockVariable = "CertRenewal_$($ObjectName)_Lock"
        try {
            Remove-AutomationVariable -Name $lockVariable -ErrorAction SilentlyContinue
            Write-Output "Released renewal lock for certificate $ObjectName"
        } catch {
            Write-Output "Could not release renewal lock (this is not critical)"
        }
    }
